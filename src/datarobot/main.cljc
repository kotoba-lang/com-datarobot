(ns datarobot.main
  "Kotodama WASM entrypoint for the Datarobot clean-room actor (L4) — Clojure port.
  ...
  facts (`datarobot.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "datarobot")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"datarobot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "datarobot-compat" :tier tier :entities entities} 200])
...
(defn start! [] :datarobot-compat/ready)
